...
export default function AnimatedGradient({
  ...
  const params = useMemo(() => {
    ...
    const presetName = config?.preset || (isDarkMode ? "Prism" : "PrismLight");
    const preset = presets[presetName] || (isDarkMode ? presets.Prism : presets.PrismLight);
    return {
      ...preset,
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      // Guard: On mobile, ignore vertical address-bar shifts during scrolling to prevent WebGL buffer recreation
      if (
        lastWidth > 0 &&
        Math.abs(width - lastWidth) < 15 &&
        (isMobile ? Math.abs(height - lastHeight) < 120 : Math.abs(height - lastHeight) < 10)
      ) {
        return;
      }
    ...
    const animate = (time) => {
      ...
      const isMobile = window.innerWidth < 768;
      const effectiveDPR = Math.min(window.devicePixelRatio || 1, isMobile ? 1.2 : 1.5);

      gl.uniform1f(uniforms.u_time, elapsed * speed + p.offset * 0.01);
      gl.uniform2f(uniforms.u_resolution, canvas.width, canvas.height);
      gl.uniform1f(uniforms.u_pixelRatio, effectiveDPR);
      gl.uniform1f(uniforms.u_scale, p.scale);
      gl.uniform1f(uniforms.u_rotation, (p.rotation * Math.PI) / 180);

      const targetC1 = hexToRgba(p.color1);
      const targetC2 = hexToRgba(p.color2);
      const targetC3 = hexToRgba(p.color3);
      ...
      if (!currentColorsRef.current) {
        ...
        const lerp = 0.08;
        for (let i = 0; i < 4; i++) {
          currentColorsRef.current.c1[i] += (targetC1[i] - currentColorsRef.current.c1[i]) * lerp;
          currentColorsRef.current.c2[i] += (targetC2[i] - currentColorsRef.current.c2[i]) * lerp;
          currentColorsRef.current.c3[i] += (targetC3[i] - currentColorsRef.current.c3[i]) * lerp;
        }
      }

      const c1 = currentColorsRef.current.c1;
      const c2 = currentColorsRef.current.c2;
      const c3 = currentColorsRef.current.c3;
